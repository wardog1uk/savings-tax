    ...
    <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center py-2 border-b">
          ...
          <span className="font-medium">{formatCurrency(results.personalSavingsAllowance)}</span>
        </div>
        
        <div className="flex justify-between items-center py-2 border-b">
          <span className="text-gray-600">Starting Rate for Savings:</span>
          <span className="font-medium">{formatCurrency(results.startingSavingsRate)}</span>
        </div>
        
        <div className="flex justify-between items-center py-2 border-b">
          <span className="text-gray-600">Taxable Amount:</span>
          <span className="font-medium">{formatCurrency(results.taxableAmount)}</span>
        </div>

        {results.basicRateTax > 0 && (
          <div className="flex justify-between items-center py-2 border-b">
            <span className="text-gray-600">Basic Rate Tax (20%):</span>
            <span className="font-medium">{formatCurrency(results.basicRateTax)}</span>
          </div>
        )}
        ...
        {results.higherRateTax > 0 && (
          <div className="flex justify-between items-center py-2 border-b">
            ...
            <span className="font-medium">{formatCurrency(results.higherRateTax)}</span>
          </div>
        )}
        ...
        {results.additionalRateTax > 0 && (
          <div className="flex justify-between items-center py-2 border-b">
            ...
            <span className="font-medium">{formatCurrency(results.additionalRateTax)}</span>
          </div>
        )}

        <div className="flex justify-between items-center py-3 border-t-2 border-blue-500">
          <span className="text-lg font-semibold text-gray-800">Total Tax Due:</span>
